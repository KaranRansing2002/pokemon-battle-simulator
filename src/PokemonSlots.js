...
function PokemonSlots(props) {
  ...
  const {setSelectedPokemon,setInd,setMoves,onClickHandle} = props
        ...
        <div className={`h-${height==undefined ? 'auto' : height} w-${width!=undefined ? width : '[55%]'} ${props.colors} border-black border-2 flex justify-center items-center`} >
      {
        arr.map((id,index)=>{
            return (
                ...
                <div className={`h-[94%] w-1/6 border-black border bg-green-200 m-1 hover:scale-[1.11]`} onClick={onClickHandle != undefined && (() => { setSelectedPokemon(team[id].name); setInd(index); setMoves(team[id].moves)})}>
